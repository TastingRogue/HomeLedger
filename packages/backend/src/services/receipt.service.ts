...
export interface ReceiptAnalysis {
  ...
  currency: string; documentType: 'receipt' | 'invoice' | 'cfdi' | 'unknown';
  status: 'pending' | 'processing' | 'completed' | 'failed'; confidence: number; rawText: string | null;
  uuid: string | null; issuerRfc: string | null; issuerName: string | null; createdAt: string; updatedAt: string; items: ReceiptItem[];
}
interface ParsedReceipt extends Omit<ReceiptAnalysis, 'id' | 'attachmentId' | 'userId' | 'transactionId' | 'status' | 'createdAt' | 'updatedAt' | 'items'> { items: Omit<ReceiptItem, 'id' | 'analysisId'>[]; }
...
function ensureTables(): void {
  getSqlite().exec(`
    CREATE TABLE IF NOT EXISTS receipt_analyses (
      ...
      issuer_name TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL
    );
...
function parsePlainText(text: string): ParsedReceipt {
  const totalText = firstMatch(text, [/(?:TOTAL(?:\s+A\s+PAGAR)?|IMPORTE\s+TOTAL)[^\d]{0,20}(\$?\s*[\d,]+(?:\.\d{2})?)/i]);
  ...
  return { merchant, receiptDate: parseDate(dateText), subtotal: numberValue(subtotalText), tax: numberValue(taxText), total: numberValue(totalText), currency: 'MXN', documentType: /factura|cfdi/i.test(text) ? 'invoice' : 'receipt', confidence: totalText ? 0.65 : 0.25, rawText: text, uuid: null, issuerRfc: null, issuerName: null, items: [] };
}
...
export class ReceiptService {
  static list(userId: number): ReceiptAnalysis[] { ensureTables(); return (getSqlite().prepare('SELECT * FROM receipt_analyses WHERE user_id = ? ORDER BY created_at DESC').all(userId) as Record<string, unknown>[]).map(row => this.hydrate(row)); }
  static get(id: number, userId: number): ReceiptAnalysis | null { ensureTables(); const row = getSqlite().prepare('SELECT * FROM receipt_analyses WHERE id = ? AND user_id = ?').get(id, userId) as Record<string, unknown> | undefined; return row ? this.hydrate(row) : null; }
  static async analyze(attachmentId: number, userId: number): Promise<ReceiptAnalysis> {
    ...
    try {
      ...
      sqlite.prepare("UPDATE receipt_analyses SET merchant = ?, receipt_date = ?, subtotal = ?, tax = ?, total = ?, currency = ?, document_type = ?, status = 'completed', confidence = ?, raw_text = ?, uuid = ?, issuer_rfc = ?, issuer_name = ?, updated_at = ? WHERE id = ? AND user_id = ?").run(parsed.merchant, parsed.receiptDate, parsed.subtotal, parsed.tax, parsed.total, parsed.currency, parsed.documentType, parsed.confidence, parsed.rawText, parsed.uuid, parsed.issuerRfc, parsed.issuerName, new Date().toISOString(), analysisId, userId);
    ...
    } catch (error) { sqlite.prepare("UPDATE receipt_analyses SET status = 'failed', updated_at = ? WHERE id = ? AND user_id = ?").run(new Date().toISOString(), analysisId, userId); throw error; }
  }
  private static parseAttachment(attachment: AttachmentRecord): ParsedReceipt {
    const filePath = AttachmentService.getFilePath(attachment); if (!fs.existsSync(filePath)) throw new Error('Archivo no encontrado en disco');
    if (attachment.mimeType === 'text/xml' || attachment.mimeType === 'application/xml' || path.extname(attachment.filename).toLowerCase() === '.xml') return parseCfdi(fs.readFileSync(filePath, 'utf8'));
    if (attachment.mimeType === 'application/pdf') { const text = extractPdfText(filePath); if (text?.trim()) return parsePlainText(text); throw new Error('No se pudo extraer texto del PDF. Instala pdftotext o agrega un motor OCR local.'); }
