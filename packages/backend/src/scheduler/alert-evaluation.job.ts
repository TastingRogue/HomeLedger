...
const JOB_NAME = 'alert-evaluation';
...
export function startAlertEvaluationJob(): ScheduledTask {
  ...
  (async () => {
    ...
    try {
      ...
      recordJobRun(JOB_NAME, 'success', Date.now() - startTime, `startup: ${totalAlerts} alerts generated for ${allUsers.length} users`);
    } catch (error) {
      /* startup evaluation is best-effort */
      recordJobRun(JOB_NAME, 'error', Date.now() - startTime, `startup failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  })();
  ...
  task = schedule('0 * * * *', async () => {
    ...
    try {
      ...
      recordJobRun(JOB_NAME, 'success', elapsed, `${totalAlerts} alerts generated for ${allUsers.length} users`);
    } catch (error) {
      const elapsed = Date.now() - startTime;
