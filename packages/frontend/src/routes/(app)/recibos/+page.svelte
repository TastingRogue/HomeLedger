<script lang="ts">
  ...
  let uploading=$state(false); let deleting=$state(false); let fileInput=$state<HTMLInputElement|null>(null);
  ...
  onMount(load);
  async function load(){ loading=true; error=''; try { const [r,a]=await Promise.all([listReceipts(),listAttachments()]); receipts=r; attachments=a; } catch(e){ error=e instanceof Error?e.message:'No se pudieron cargar los documentos'; } finally{loading=false;} }
  ...
  async function handleUpload(e:Event){
    ...
    try{ await uploadAttachment(file); await load(); }
    catch(err){ error=err instanceof Error?err.message:'No se pudo subir el archivo'; }
    finally{ uploading=false; if(fileInput)fileInput.value=''; }
  }
  async function download(){ if(!selected)return; error=''; try{ await apiDownload(selected.attachmentId, selected.filename??`comprobante-${selected.attachmentId}`); }catch(err){ error=err instanceof Error?err.message:'No se pudo descargar'; } }
  async function remove(){
    ...
    catch(err){ error=err instanceof Error?err.message:'No se pudo eliminar'; }
...
<div class="toolbar"><input bind:value={search} placeholder="Buscar comercio, RFC, archivo..."/><button class="secondary" onclick={load} disabled={loading}>Actualizar</button><button class="upload-btn" onclick={triggerUpload} disabled={uploading}>{uploading?$t('receipts.uploading'):$t('receipts.upload')}</button><input bind:this={fileInput} type="file" accept="image/jpeg,image/png,image/webp,image/gif,application/pdf,.xml" onchange={handleUpload} style="display:none" /></div>
