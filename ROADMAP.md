...
- [ ] API `/api/v1` and backup format explicitly frozen as stable — **decision + doc step for the release.** The surface is ready to freeze (versioned `/api/v1` routes; backup carries `version` + a major-version compatibility gate in `validateBackup`). Needs an explicit written "stable, breaking changes require a major bump" statement (README/docs) before tagging 1.0.0.
- [ ] CHANGELOG updated; version bumped to 1.0.0 across all manifests — **release action** (see the "Version bump checklist" below: root + 3 package.json, `APP_VERSION` in backup.service.ts, `ha-addon/config.yaml`, HA `manifest.json` + `sw_version`, CHANGELOG, tag). Best done as the actual release cut once the two items above are signed off.
