...
- [~] Accessibility pass — **static pass done ✅; manual assistive-tech verification remains.**
  ...
  - [ ] **Remaining (manual, needs a running instance + assistive tech):** full **focus management** (move focus into a dialog on open, trap it, return it to the trigger on close — deliberately not done statically as it needs runtime testing), screen-reader walkthrough (NVDA/VoiceOver), full keyboard-only navigation, and measured **color-contrast ratios** against WCAG AA. These can't be verified from static analysis alone.
...
- [x] All tests pass (backend green; basic frontend coverage) ✅ — **508 passing** (458 backend/shared + 50 frontend). Frontend now has real coverage: pure-logic tests (format/i18n/API-client) **plus** component/DOM smoke tests on the critical flows (login, create-transaction, backup, Dropdown). `npm run test` runs the whole monorepo green.
